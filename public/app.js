...
function photoStep() { return `<div class="step"><h2>Upload Profile Photo</h2><p>Upload a PNG, JPEG, GIF, WebP, or other standard image file, or paste an image URL.</p>${fileInput("profileUpload", "Choose image file", "image/*")}<input data-field="photoUrl" value="${esc(state.onboarding.photoUrl)}" placeholder="Image URL or uploaded data URL"></div>`; }
...
function bindScreen() {
  ...
  document.querySelector("#editPhotoUpload")?.addEventListener("change", async (event) => { const file = event.target.files?.[0]; if (file) { state.profileEdit = state.profileEdit || structuredClone(state.user.profile); state.profileEdit.photoUrl = await readFileAsDataUrl(file); render(); } });
  document.querySelector("#saveProfile")?.addEventListener("click", async () => { const current = state.user.profile; await saveProfile({ ...current, photoUrl: document.querySelector("#editPhoto").value, fullName: document.querySelector("#editName").value, industry: document.querySelector("#editIndustry").value, businessType: state.profileEdit?.businessType || asArray(current.businessType), title: document.querySelector("#editTitle").value, socialLinks: document.querySelector("#editSocial").value }); });
