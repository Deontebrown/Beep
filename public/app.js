const state = {
  ...
  tab: "home",
  status: "",
  data: { events: [], connections: [], feed: [], messages: [], swaps: [], toolboxDocuments: [], badges: [], ads: [], adSettings: { frequency: 4 }, admin: null },
  selectedEvent: null,
  selectedThread: null,
...
async function loadAppData() {
  const [events, connections, feed, messages, swaps, toolbox, badges] = await Promise.all([api("/api/events"), api("/api/connections"), api("/api/feed"), api("/api/messages"), api("/api/skill-swaps"), api("/api/toolbox").catch(() => ({ documents: [] })), api("/api/badges").catch(() => ({ badges: [] }))]);
  state.data = { ...state.data, events: events.events || [], connections: connections.connections || [], feed: feed.posts || [], messages: messages.messages || [], swaps: swaps.swaps || [], toolboxDocuments: toolbox.documents || [], badges: badges.badges || [], ads: feed.ads || [], adSettings: feed.adSettings || { frequency: 4 } };
...
function messagesScreen() {
  ...
  return `<section class="screen chat-screen"><button class="secondary" id="backToInbox">Back to messages</button><button class="chat-profile-header" data-profile="${selected.user.id}">${avatar(selected.user.profile)}<span><strong>${esc(selected.user.profile?.fullName)}</strong><small>${esc(selected.user.profile?.title || "View profile")}</small></span>${onlineDot(selected.user.onlineStatus)}</button><p class="status">${esc(state.status)}</p><div class="message-list">${threadMessages.map((message) => { const mine = message.senderId === state.user.id; return `<div class="bubble ${mine ? "mine" : ""}"><p>${esc(message.body)}</p><small>${esc(message.sender.profile?.fullName)}${message.editedAt ? " · edited" : ""}</small>${mine ? `<div class="own-actions"><button title="Edit message" data-edit-message="${message.id}">⋯ Edit</button><button title="Delete message" data-delete-message="${message.id}">Delete</button></div>` : ""}</div>`; }).join("") || '<p class="empty-thread">No messages yet. Say hello.</p>'}</div><div class="message-composer"><input id="messageBody" placeholder="Write a private message"><button class="primary" id="sendMessage">Send</button></div></section>`;
}
function renderAdCard(ad) { return `<article class="feed ad-card"><img class="flyer" src="${esc(ad.imageUrl)}" alt="Ad image"><p>${esc(ad.copy || "Sponsored")}</p><button class="mini" data-ad-cta="${esc(ad.ctaUrl)}">${esc(ad.ctaText || "Learn more")}</button><span class="ad-label">Ad</span></article>`; }
function feedScreen() { const activeAds = (state.data.ads || []).filter((ad) => ad.active !== false); const freq = Math.max(1, Number(state.data.adSettings?.frequency || 4)); const cards = []; state.data.feed.forEach((post, index) => { const comments = post.comments || []; const open = state.openCommentsPostId === post.id; const author = post.author || { id: "", profile: { fullName: "Prime Member" } }; const mine = author.id === state.user.id; cards.push(`<article class="feed"><div class="feed-head"><button class="row feed-author" data-profile="${author.id}">${avatar(author.profile)}<div><strong>${esc(author.profile?.fullName || "Prime Member")}</strong><p>${post.type === "BADGE" ? "Badge achievement" : esc(author.profile?.title)}</p></div></button>${mine ? `<div class="own-actions"><button title="Edit post" data-edit-post="${post.id}">⋯ Edit</button><button title="Delete post" data-delete-post="${post.id}">Delete</button></div>` : ""}</div><p>${esc(post.body)}</p>${post.editedAt ? `<small>Edited ${new Date(post.editedAt).toLocaleDateString([], { month: "short", day: "numeric" })}</small>` : ""}<div class="feed-actions"><small>${post.likes.length} likes</small><button class="comment-toggle" data-comments="${post.id}">${open ? "Hide" : "View"} comments (${comments.length})</button></div>${open ? `<section class="comments-panel">${comments.map((comment) => `<article class="comment-row">${avatar(comment.author?.profile)}<div><strong>${esc(comment.author?.profile?.fullName || "Prime Member")}</strong><p>${esc(comment.body)}</p></div></article>`).join("") || "<p>No comments yet.</p>"}<div class="comment-form"><input id="comment-${post.id}" placeholder="Write a comment"><button class="mini" data-comment-submit="${post.id}">Post</button></div></section>` : ""}</article>`); if ((index + 1) % freq === 0 && activeAds.length) { const ad = activeAds[Math.floor(Math.random() * activeAds.length)]; cards.push(renderAdCard(ad)); } }); return `<section class="screen"><h2>Prime Feed</h2><p>Keep posts professional and focused on wins and connections. External links, nudity, and profanity are blocked.</p><textarea id="postBody" placeholder="Share a professional win from Prime Connects."></textarea><button class="primary" id="sharePost">◆ Share win</button><p class="status">${esc(state.status)}</p>${cards.join("")}</section>`; }
...
function accountScreen() {
  ...
  const portfolio = `<h3>Portfolio</h3><p>Add up to 4 examples. Upload images from your device and add a short description for each item.</p>${portfolioGrid(items, true)}${items.length < 4 ? `<div class="portfolio-add"><label class="field upload-field">Portfolio image<input id="portfolioImageUpload" type="file" accept="image/png,image/jpeg,image/jpg,image/gif,image/webp,image/*"></label><input id="portfolioDescription" placeholder="Short description"><button class="primary" id="addPortfolioItem">Add portfolio item</button></div>` : ""}<button class="primary" id="savePortfolio">Save portfolio</button>`;
  return `<section class="screen"><div class="profile"><label class="profile-photo-upload" title="Tap to change profile picture">${avatar(profile)}<input id="editPhotoUpload" type="file" accept="image/png,image/jpeg,image/jpg,image/gif,image/webp,image/*"><span>Tap photo to change</span></label><h2>${esc(profile.fullName)}</h2><p>${esc(profile.title)}</p></div><div class="badge-grid">${state.user.badges.map(badgeChip).join("") || "<p>No badges yet.</p>"}</div>${profileTabs(state.accountTab)}${state.accountTab === "portfolio" ? portfolio : overview}<button class="secondary" id="logout">Log out</button><div class="account-danger-zone"><button id="deactivateAccount">Deactivate Account</button><button id="deleteAccount">Delete Account</button></div></section>`;
}
...
function adminScreen() {
  ...
  if (state.adminSection === "ads") return adminAdsScreen(admin);
  if (state.adminSection === "users") return adminUsersScreen(admin);
  return `<section class="screen admin-dashboard"><h2>Admin Portal</h2><p>Signed in as networking@primeconnectsindy.com. Choose a management area below.</p><div class="admin-grid"><button class="admin-card" data-admin-section="events"><span>${navIcon("events")}</span><strong>Manage Events</strong><small>Create event details, flyers, RSVP links, dress code, and descriptions.</small></button><button class="admin-card" data-admin-section="badges"><span>${navIcon("admin")}</span><strong>Manage Badges</strong><small>Create manual or connection-count achievement badges.</small></button><button class="admin-card" data-admin-section="toolbox"><span>${navIcon("toolbox")}</span><strong>Prime Business Toolbox</strong><small>Upload and review categorized business documents.</small></button><button class="admin-card" data-admin-section="ads"><span>${navIcon("feed")}</span><strong>Manage Ads</strong><small>Create, schedule, and rotate feed ads.</small></button><button class="admin-card" data-admin-section="users"><span>${navIcon("connections")}</span><strong>Manage Users</strong><small>Review members and remove accounts when needed.</small></button></div></section>`;
}
...
function adminBadgesScreen(admin) {
  ...
  return `<section class="screen">${adminBackButton()}<h2>Manage Badges</h2><p>Create or edit automatic if/then badge rules, upload badge icons, and manually assign badges to members.</p><h3>${state.adminBadgeEdit ? "Edit badge" : "Create badge"}</h3><select id="adminBadgeSelect"><option value="">Create new badge</option>${admin.badges.map((badge) => `<option value="${esc(badge.name)}" ${state.adminBadgeEdit?.name === badge.name ? "selected" : ""}>${esc(badge.name)}</option>`).join("")}</select><input id="badgeName" placeholder="Badge name" value="${esc(editing.name)}"><label class="field">If / then trigger<select id="badgeCriteria"><option value="manual" ${editing.criteriaType === "manual" ? "selected" : ""}>Manual assignment only</option><option value="events" ${["events", "attendances"].includes(editing.criteriaType) ? "selected" : ""}>If a user attends X events</option><option value="connections" ${editing.criteriaType === "connections" ? "selected" : ""}>If a user makes X connections</option></select></label><input id="badgeCriteriaCount" type="number" min="0" placeholder="Trigger number, e.g. 3" value="${esc(editing.criteriaCount || 0)}">${fileInput("badgeIconUpload", "Upload badge icon/image", "image/png,image/jpeg,image/jpg,image/gif,image/webp,image/*")}<input id="badgeIcon" placeholder="Badge icon URL or uploaded image data" value="${esc(editing.iconUrl || "")}"><div id="badgeIconPreview" class="badge-icon-preview">${editing.iconUrl ? `<img src="${esc(editing.iconUrl)}" alt="${esc(editing.name || "Badge")} icon">` : ""}</div><button class="primary" id="createBadge">${state.adminBadgeEdit ? "Save badge changes" : "Create badge"}</button><p class="status">${esc(state.status)}</p><h3>Manual badge assignment</h3><p>Select a user and one or more badges to assign immediately.</p><select id="assignUser"><option value="">Choose user</option>${admin.users.map((user) => `<option value="${user.id}">${esc(user.profile?.fullName || user.email)}</option>`).join("")}</select><div class="badge-assign-grid">${admin.badges.map((badge) => `<label class="badge-assign-option"><input type="checkbox" data-assign-badge="${esc(badge.name)}"><span>${esc(badge.name)}</span></label>`).join("") || "<p>No badges created yet.</p>"}</div><button class="secondary" id="assignBadgeButton">Assign selected badges</button><h4>Users and badges</h4><div class="admin-user-badge-list">${admin.users.map((member) => `<article class="admin-user-badge-row"><strong>${esc(member.profile?.fullName || member.email)}</strong><small>${esc(member.email)}</small><div class="badge-grid compact-badges">${(member.badges || []).map(badgeChip).join("") || "<p>No badges yet.</p>"}</div></article>`).join("")}</div><h3>Existing badges</h3><div class="badge-admin-list">${admin.badges.map((badge) => `<article class="badge-admin-row"><div>${badge.iconUrl ? `<img src="${esc(badge.iconUrl)}" alt="${esc(badge.name)} icon">` : "🏆"}</div><span><strong>${esc(badge.name)}</strong><small>${esc(ruleText(badge))}</small></span><button class="mini" data-edit-badge="${esc(badge.name)}">Edit</button></article>`).join("")}</div></section>`;
}
function adminToolboxScreen(admin) {
  return `<section class="screen">${adminBackButton()}<h2>Prime Business Toolbox</h2><p>Upload categorized resources for members. Supported formats include PDF, Word, Excel, PowerPoint, text, CSV, RTF, and OpenDocument files.</p><input id="toolboxTitle" placeholder="Document title"><select id="toolboxCategory">${toolboxCategories.map((category) => `<option>${category}</option>`).join("")}</select><textarea id="toolboxDescription" maxlength="220" placeholder="Optional short description"></textarea>${fileInput("toolboxFile", "Upload document (PDF, Word, Excel, PowerPoint, etc.)", toolboxAccept)}<button class="primary" id="uploadToolboxDoc">Upload document</button><p class="status">${esc(state.status)}</p><div class="toolbox-admin-list">${(admin.toolboxDocuments || []).map((doc) => `<article class="toolbox-doc"><div><strong>${esc(doc.title)}</strong><p>${esc(doc.category)} · ${esc(doc.fileName)}</p></div><button class="mini danger" data-delete-toolbox="${doc.id}">Delete</button></article>`).join("") || "<p>No toolbox documents yet.</p>"}</div></section>`;
}

function adminAdsScreen(admin) {
  const editing = state.adminAdEdit || { imageUrl: "", copy: "", ctaText: "Learn more", ctaUrl: "", startDate: "", endDate: "", active: true };
  return `<section class="screen">${adminBackButton()}<h2>Manage Ads</h2><p>Create and rotate feed ads shown between posts.</p><select id="adminAdSelect"><option value="">Create new ad</option>${(admin.ads || []).map((ad) => `<option value="${ad.id}" ${state.adminAdEdit?.id === ad.id ? "selected" : ""}>${esc(ad.copy?.slice(0, 40) || ad.id)}</option>`).join("")}</select>${fileInput("adminAdImageUpload", "Upload ad image", "image/png,image/jpeg,image/jpg,image/gif,image/webp,image/*")}<input id="adminAdImage" placeholder="Uploaded image data" value="${esc(editing.imageUrl)}"><textarea id="adminAdCopy" placeholder="Ad copy">${esc(editing.copy)}</textarea><input id="adminAdCtaText" placeholder="CTA button text" value="${esc(editing.ctaText)}"><input id="adminAdCtaUrl" placeholder="CTA URL" value="${esc(editing.ctaUrl)}"><label class="field">Start date<input id="adminAdStart" type="date" value="${esc(editing.startDate)}"></label><label class="field">End date<input id="adminAdEnd" type="date" value="${esc(editing.endDate)}"></label><label class="admin-toggle"><input id="adminAdActive" type="checkbox" ${editing.active ? "checked" : ""}> Active</label><label class="field">Ad frequency (every X posts)<input id="adminAdFrequency" type="number" min="1" value="${Number(admin.adSettings?.frequency || 4)}"></label><button class="primary" id="saveAd">${editing.id ? "Save ad" : "Create ad"}</button>${editing.id ? `<button class="secondary danger-outline" id="deleteAd">Delete ad</button>` : ""}<button class="secondary" id="saveAdFrequency">Save frequency</button><h3>All ads</h3><div class="admin-event-list">${(admin.ads || []).map((ad) => `<article class="admin-event-row"><div><strong>${esc(ad.copy || "Ad")}</strong><p>${ad.active ? "Active" : "Inactive"} · ${esc(ad.startDate || "No start")} to ${esc(ad.endDate || "No end")}</p></div><div><button class="mini" data-edit-ad="${ad.id}">Edit</button><button class="mini danger" data-delete-ad="${ad.id}">Delete</button></div></article>`).join("") || "<p>No ads yet.</p>"}</div></section>`;
}
...
function bindScreen() {
  ...
  document.querySelector("#sendMessage")?.addEventListener("click", async () => { await api("/api/messages", { method: "POST", body: JSON.stringify({ receiverId: state.selectedThread, body: document.querySelector("#messageBody").value }) }); state.status = ""; await loadAppData(); render(); });
  document.querySelectorAll("[data-edit-message]").forEach((button) => button.onclick = async () => { const message = state.data.messages.find((item) => item.id === button.dataset.editMessage); const body = prompt("Edit your message", message?.body || ""); if (body === null) return; try { await api("/api/messages/edit", { method: "POST", body: JSON.stringify({ messageId: button.dataset.editMessage, body }) }); state.status = "Message updated."; await loadAppData(); render(); } catch (error) { setStatus(error.message); } });
  document.querySelectorAll("[data-delete-message]").forEach((button) => button.onclick = async () => { if (!confirm("Delete this message?")) return; try { await api("/api/messages/delete", { method: "POST", body: JSON.stringify({ messageId: button.dataset.deleteMessage }) }); state.status = "Message deleted."; await loadAppData(); render(); } catch (error) { setStatus(error.message); } });
  ...
  document.querySelectorAll("[data-edit-post]").forEach((button) => button.onclick = async () => { const post = state.data.feed.find((item) => item.id === button.dataset.editPost); const body = prompt("Edit your post", post?.body || ""); if (body === null) return; try { await api("/api/feed/edit", { method: "POST", body: JSON.stringify({ postId: button.dataset.editPost, body }) }); state.status = "Post updated."; await loadAppData(); render(); } catch (error) { setStatus(error.message); } });
  document.querySelectorAll("[data-delete-post]").forEach((button) => button.onclick = async () => { if (!confirm("Delete this post?")) return; try { await api("/api/feed/delete", { method: "POST", body: JSON.stringify({ postId: button.dataset.deletePost }) }); state.status = "Post deleted."; await loadAppData(); render(); } catch (error) { setStatus(error.message); } });
  ...
  document.querySelector("#addPortfolioItem")?.addEventListener("click", async () => { const file = document.querySelector("#portfolioImageUpload")?.files?.[0]; const description = document.querySelector("#portfolioDescription").value.trim(); if (!file) return setStatus("Choose a portfolio image to upload."); const image = await readFileAsDataUrl(file); state.profileEdit = state.profileEdit || structuredClone(state.user.profile); state.profileEdit.portfolioItems = [...profilePortfolio(state.profileEdit), { image, description }].slice(0, 4); render(); });
  ...
  document.querySelector("#deactivateAccount")?.addEventListener("click", async () => { if (!confirm("Are you sure you want to deactivate your account? You can reactivate it by logging back in.")) return; await api("/api/account/deactivate", { method: "POST" }); state.user = null; state.tab = "home"; render(); });
  document.querySelector("#deleteAccount")?.addEventListener("click", async () => { if (!confirm("Are you sure you want to delete your account? This cannot be undone.")) return; if (!confirm("This will permanently delete all your data, posts, messages and connections forever. Are you absolutely sure?")) return; await api("/api/account/delete", { method: "POST" }); state.user = null; state.tab = "home"; render(); });
  ...
  document.querySelectorAll("[data-delete-toolbox]").forEach((button) => button.onclick = async () => { if (!confirm("Delete this toolbox document?")) return; try { await api("/api/admin/toolbox/delete", { method: "POST", body: JSON.stringify({ id: button.dataset.deleteToolbox }) }); state.status = "Document removed from the Prime Business Toolbox."; await loadAppData(); render(); } catch (error) { setStatus(error.message); } });
  ...
  document.querySelector("#assignBadgeButton")?.addEventListener("click", async () => { const userId = document.querySelector("#assignUser").value; const badgeNames = Array.from(document.querySelectorAll("[data-assign-badge]:checked")).map((input) => input.dataset.assignBadge); if (!userId || !badgeNames.length) return setStatus("Select a user and at least one badge."); for (const badgeName of badgeNames) await api("/api/admin/badges/assign", { method: "POST", body: JSON.stringify({ badgeName, userId }) }); state.status = `${badgeNames.length} badge${badgeNames.length > 1 ? "s" : ""} assigned.`; await loadAppData(); render(); });
  ...
  document.querySelector("#adminAdSelect")?.addEventListener("change", (event) => { state.adminAdEdit = state.data.admin.ads.find((item) => item.id === event.target.value) || null; render(); });
  document.querySelector("#adminAdImageUpload")?.addEventListener("change", async (event) => { const file = event.target.files?.[0]; if (file) document.querySelector("#adminAdImage").value = await readFileAsDataUrl(file); });
  document.querySelector("#saveAd")?.addEventListener("click", async () => { await api("/api/admin/ads", { method: "POST", body: JSON.stringify({ id: state.adminAdEdit?.id, imageUrl: document.querySelector("#adminAdImage").value, copy: document.querySelector("#adminAdCopy").value, ctaText: document.querySelector("#adminAdCtaText").value, ctaUrl: document.querySelector("#adminAdCtaUrl").value, startDate: document.querySelector("#adminAdStart").value, endDate: document.querySelector("#adminAdEnd").value, active: document.querySelector("#adminAdActive").checked }) }); state.adminAdEdit = null; await loadAppData(); render(); });
  document.querySelector("#saveAdFrequency")?.addEventListener("click", async () => { await api("/api/admin/ads/settings", { method: "POST", body: JSON.stringify({ frequency: document.querySelector("#adminAdFrequency").value }) }); await loadAppData(); render(); });
  document.querySelector("#deleteAd")?.addEventListener("click", async () => { await api("/api/admin/ads/delete", { method: "POST", body: JSON.stringify({ id: state.adminAdEdit?.id }) }); state.adminAdEdit = null; await loadAppData(); render(); });
  document.querySelectorAll("[data-edit-ad]").forEach((button) => button.onclick = () => { state.adminAdEdit = state.data.admin.ads.find((item) => item.id === button.dataset.editAd) || null; render(); });
  document.querySelectorAll("[data-delete-ad]").forEach((button) => button.onclick = async () => { await api("/api/admin/ads/delete", { method: "POST", body: JSON.stringify({ id: button.dataset.deleteAd }) }); await loadAppData(); render(); });
  document.querySelectorAll("[data-ad-cta]").forEach((button) => button.onclick = () => window.open(button.dataset.adCta, "_blank", "noopener"));
