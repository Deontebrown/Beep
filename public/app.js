...
function bindScreen() {
  ...
  document.querySelector("#saveProfile")?.addEventListener("click", async () => { const current = state.user.profile; await saveProfile({ ...current, photoUrl: current.photoUrl, fullName: document.querySelector("#editName").value, industry: document.querySelector("#editIndustry").value, businessType: state.profileEdit?.businessType || asArray(current.businessType), title: document.querySelector("#editTitle").value, socialLinks: document.querySelector("#editSocial").value }, "account"); });
