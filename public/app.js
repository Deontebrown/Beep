const state = {
  ...
  tab: "home",
  status: "",
  data: { events: [], connections: [], feed: [], messages: [], swaps: [], toolboxDocuments: [], admin: null },
  selectedEvent: null,
  selectedThread: null,
...
async function loadAppData() {
  const [events, connections, feed, messages, swaps, toolbox] = await Promise.all([api("/api/events"), api("/api/connections"), api("/api/feed"), api("/api/messages"), api("/api/skill-swaps"), api("/api/toolbox")]);
  state.data = { ...state.data, events: events.events, connections: connections.connections, feed: feed.posts, messages: messages.messages, swaps: swaps.swaps, toolboxDocuments: toolbox.documents };
...
function adminEventsScreen(admin) {
  ...
  return `<section class="screen">${adminBackButton()}<h2>Manage Events</h2><p>Create or update event details, flyers, RSVP links, and attendee-facing information.</p><select id="adminEventSelect"><option value="">Create new event</option>${admin.events.map((event) => `<option value="${event.id}" ${state.adminEventEdit?.id === event.id ? "selected" : ""}>${esc(event.name)}</option>`).join("")}</select><input id="adminName" placeholder="Event name" value="${esc(editing.name)}"><input id="adminDate" placeholder="Event date ISO" value="${esc(editing.date)}"><input id="adminLocation" placeholder="Location" value="${esc(editing.location)}"><input id="adminDress" placeholder="Dress code" value="${esc(editing.dressCode)}">${fileInput("adminFlyerUpload", "Upload flyer (PNG, JPEG, PDF, etc.)", "image/*,application/pdf")}<input id="adminFlyer" placeholder="Event flyer URL or uploaded file data" value="${esc(editing.flyerUrl)}"><input id="adminRsvp" placeholder="Event RSVP URL" value="${esc(editing.rsvpUrl)}"><textarea id="adminDescription" placeholder="Description">${esc(editing.description)}</textarea><button class="primary" id="saveEvent">Save event</button></section>`;
}
function adminBadgesScreen(admin) {
  return `<section class="screen">${adminBackButton()}<h2>Manage Badges</h2><p>Create badges that can be awarded manually later or earned automatically by connection milestones.</p><input id="badgeName" placeholder="New badge name"><select id="badgeCriteria"><option value="manual">Manual award later</option><option value="connections">Connection count</option></select><input id="badgeCriteriaCount" type="number" min="0" placeholder="Requirement count, e.g. 100"><button class="primary" id="createBadge">Create badge</button><div class="tags">${admin.badges.map((badge) => `<em>${esc(badge.name || badge)}${badge.criteriaType === "connections" ? ` · ${badge.criteriaCount} connections` : ""}</em>`).join("")}</div></section>`;
}
...
function adminUsersScreen(admin) {
  return `<section class="screen">${adminBackButton()}<h2>Manage Users</h2><p>Review member accounts and remove users when necessary.</p>${admin.users.map((user) => `<article class="row"><div>${avatar(user.profile)}</div><div><strong>${esc(user.profile?.fullName || user.email)} ${onlineDot(user.onlineStatus)}</strong><p>${esc(user.email)}</p></div>${user.email !== state.user.email ? `<button class="mini danger" data-remove-user="${user.id}">Remove</button>` : ""}</article>`).join("")}</section>`;
}
